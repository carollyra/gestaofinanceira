...
import { CHART_ANIMATION_MS, CHART_SERIES_STAGGER_MS, springEasing } from '@/utils/motion';
...
export function IncomeExpenseChart({ data }: { data: EvolutionPoint[] }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        {!visible ? (
          <BarChartSkeleton bars={data.length} />
        ) : (
          <ResponsiveContainer>
            ...
            >
              ...
              {INCOME_EXPENSE_SERIES.map((series, index) => (
                <Bar
                  key={series.key}
                  dataKey={series.key}
                  name={series.name}
                  ...
                  maxBarSize={24}
                  radius={[4, 4, 0, 0]}
                  // Bars grow from the baseline; the second series starts a beat later
